...
dataToUpdUser = {'name': "Alah", 'age': 64}
...
def test_similarPosts():
    ...
    found = False
    for post in posts:
        if post["author"] == "Maxim":
            found = True
            break
    assert found, "Author 'Maxim' not found in the response"
...
@allure.feature('Get user Hater')
@allure.story('есть ли автор Hater в комментариях')
def test_photoProfile():
    response = requests.get('http://localhost:3000/comments')
    authors = response.json()
    for author in authors:
        assert "Hater" not in author, "Author not found in user data"
...
@allure.feature('Get user Max')
@allure.story('есть ли пользователь Max')
def test_checkUserName():
    response = requests.get('http://localhost:3000/users')
    users = response.json()
    for user in users:
        assert "Max" not in user, "Author not found in user data"